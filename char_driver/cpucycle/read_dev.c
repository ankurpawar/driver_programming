...
ssize_t read_dev(struct file *filep,char __user *buf,size_t count,loff_t *f_pos)
{
	...
	wait_event_interruptible(lsculldev->dev_wait_q, data_size > 0);
	...
	rdtscl(rb);
	#ifdef DEBUG
	printk(KERN_INFO "ra=%lu rb=%lu after read,%d bytes present in scull,offset = %d\n",ra,rb,data_size,(int)*f_pos);
	#endif
	
	#ifdef DEBUG
	printk(KERN_INFO "END: %s \n",__func__);
	#endif
	lsculldev->r_cycles = (long)rb - (long)ra;
	return bytes_read;
ERR:
	up(&lsculldev->sem);
	return -1;
