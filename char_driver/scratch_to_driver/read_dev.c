...
ssize_t read_dev(struct file *filep,char __user *buf,size_t count,loff_t *f_pos)
{
	...
	int byte_pend, to_read, ret, i, bytes_read = 0;
	int qset_num,quant_num,qoff,item,nobtr = 0;
	...
	if (!count) {
		#ifdef DEBUG
		printk(KERN_INFO "0 bytes to read\n");
		#endif
		goto RET;
	}
	...
	while (bytes_read < nobtr && *f_pos < data_size) {
